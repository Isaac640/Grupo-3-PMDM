...
class LoginStorageHelper(context: Context) {
    ...
    private val androidId: String = Settings.Secure.ANDROID_ID
    ...
    fun getLogin(): List<String> {
        var loginData: MutableList<String> = mutableListOf()

        var hashList = String(getData(androidId) ?: ByteArray(0)).split('-')

        println(hashList)

        if (hashList.size == 1) return loginData

        val now: Long = Calendar.getInstance().time.time
        val past: Long = hashList[0].toLong()
        val days: Long = TimeUnit.MILLISECONDS.toDays(now - past)

        println("Han pasado: $days días desde que guardaste la clave")

        if (days < 90) loginData.addAll(hashList.subList(1, 3)) else removeLogin()
        return loginData
    }

    fun saveLogin(user: String, hash: String) {
        var now: Long = Calendar.getInstance().time.time
        var data: String = "$now-$user-$hash"

        println("Data guardada: $data")

        setData(androidId, data.toByteArray())
    }
    ...
    private fun setData(key: String?, data: ByteArray?) {
        passwordStorage?.setData(key!!, data ?: ByteArray(0))
    }

    private fun getData(key: String?): ByteArray? {
